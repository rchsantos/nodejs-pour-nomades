...
describe('check that routes are defined', function() {
  ...
  it('should be possible to access /pools/0/answers with GET', function(done) {
    server.get('/pools/0/answers').end(function(err, res) {
      if (err) { throw err; }
      res.status.should.not.equal(404);
      done();
    });
  ...
  it('should be possible to access /pools/0/answers with POST', function(done) {
    server.post('/pools/0/answers').end(function(err, res) {
      if (err) { throw err; }
      res.status.should.not.equal(404);
      done();
    });
  ...
  it('should be possible to access /users with GET', function(done) {
    server.get('/users').end(function(err, res) {
      if (err) { throw err; }
      res.status.should.not.equal(404);
      done();
    });
  });
  it('should be possible to access /users/0 with GET', function(done) {
    server.get('/users/0').end(function(err, res) {
      if (err) { throw err; }
      res.status.should.not.equal(404);
      done();
    });
